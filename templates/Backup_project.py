# # Versi Kompleks untuk export shp, Tombol export
...
# import logging
# import re # Import modul re untuk membersihkan nama kolom

# app = Flask(__name__)
# # MENAIKKAN BATAS UKURAN PAYLOAD POST
# app.config['MAX_CONTENT_LENGTH'] = 50 * 1024 * 1024 

# logging.basicConfig(level=logging.INFO) # Gunakan INFO untuk mengurangi log spam

# # Fungsi pembersihan nama kolom untuk Shapefile (max 10 karakter, alfanumerik)
# def clean_column_name(col_name):
#     """Batasi panjang kolom dan hapus karakter ilegal untuk Shapefile."""
#     cleaned = re.sub(r'[^a-zA-Z0-9_]', '', col_name)
#     # Batasi hingga 10 karakter, sesuai batasan SHP DBF
#     return cleaned[:10] 
...
#     # --- Data Statis (Marker, Polygon, TileLayer) --- (Dibiarkan sama)
...
#     try:
#         # PENTING: Pastikan folder 'static/Gambar' dan file '201874045.jpg' ada
...
#     # 🛑 MATIKAN EXPORT BAWAAN FOLIUM UNTUK MENGGUNAKAN KUSTOM DROPDOWN
#     Draw(
#         export= False, # <-- PENTING: Mengubah menjadi False
...
#     # 🌟 IMPLEMENTASI MENU EXPORT DROPDOWN (HTML/CSS/JS)
#     html_js = f'''
#     <style>
...
#             display: none; /* Awalnya sembunyikan, JS akan menampilkannya jika berhasil */
#         }}
#         .export-button {{
#             background-color: #007bff; /* Biru seperti tombol download */
#             color: white; 
#             padding: 8px 15px; 
#             cursor: pointer; 
#             border: none; 
...
#         <button class='export-button' onclick='toggleExportDropdown()'>
#             <span style="margin-right: 5px;">📥 EXPORT</span> ▼
#         </button>
...
#             document.getElementById("exportDropdownContent").classList.toggle("show-dropdown");
#         }}

#         // Tutup dropdown jika user klik di luar
#         window.onclick = function(event) {{
#             if (!event.target.matches('.export-button') && !event.target.matches('.export-button *')) {{
...
#             // Fallback: Coba ambil dari Leaflet Draw Control
#             if (!drawnItems && mapElement && mapElement._leaflet_map) {{
...
#                         // Ini adalah feature group tempat Leaflet Draw menyimpan geometri yang digambar
...
#         // Fungsi utama untuk menjalankan export
#         function performExport(format) {{
#             const drawnItems = getDrawnItems();
#             if (!drawnItems) {{
#                 alert("Kesalahan: Drawn Items FeatureGroup tidak dapat diakses.");
#                 return;
#             }}

#             const features = drawnItems.getLayers();
...
#                 // Kirim data ke Flask menggunakan form POST
#                 const form = document.createElement('form');
#                 form.method = 'POST';
                
#                 // Tentukan endpoint berdasarkan format
#                 if (format === 'shp') {{
...
#                 // Tambahkan data GeoJSON ke form tersembunyi
...
#             toggleExportDropdown(); // Tutup dropdown setelah pengiriman
#         }}

#         // Tampilkan tombol export setelah 750ms jika drawnItems ditemukan
#         setTimeout(function() {{ 
#             if (getDrawnItems()) {{
#                  document.getElementById('exportGroup').style.display = 'block';
#             }} else {{
#                 // Nonaktifkan jika gagal (opsional)
#                  console.error("Drawn Items FeatureGroup is NOT accessible. Export will not work.");
#                  const exportGroup = document.getElementById('exportGroup');
#                  exportGroup.style.display = 'block';
#                  exportGroup.querySelector('.export-button').disabled = true;
#                  exportGroup.querySelector('.export-button').textContent = "❌ EXPORT DINONAKTIFKAN";
#                  exportGroup.querySelector('.export-button').style.backgroundColor = '#888';
#             }}
#         }}, 750);
#     </script>
#     '''
...
#         css_style = '''
#              <style>
#                  /* Pindahkan Leaflet Draw control ke atas agar tidak tertutup tombol export */
...
#     # Bungkus data GeoJSON ke dalam BytesIO untuk dikirim sebagai file
...
#                 if is_folium_circle:
#                     # --- A. Fitur Circle Dikonversi Menjadi Point + Radius ---
...
#                     new_feature['properties']['radius_m'] = new_feature['properties'].pop('radius')
#                     new_feature['properties']['tipe_geom'] = 'Point' 
#                     new_feature['properties']['tipe_asal'] = 'Circle'
                    
#                     # Hapus properti Leaflet yang tidak diperlukan/bermasalah
...
#                 else:
#                     # --- B. Fitur Normal (Point, Line, Polygon) ---
#                     geom = geom_shape
...
#         # ⚠️ Terapkan pembersihan nama kolom
#         gdf_all.columns = [clean_column_name(col) for col in gdf_all.columns]
        
#         # Pisahkan GeoDataFrame berdasarkan tipe geometri
...
#                 for file in files:
#                     # Filter hanya file Shapefile dan pendukungnya
...
#Versi pembaruan yang menampilkan jarak dan radius secara otomatis
